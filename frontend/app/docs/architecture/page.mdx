export const metadata = {
  title: 'Architecture',
  description: 'Module map, the payment state machine, event flows, and the data model — with diagrams.',
...
Modules and users enter through the same HTTP surface but with different principals. Every
purchase funnels into the **payment orchestrator** — the one state machine all money movement
passes through — which fans out to subscriptions, entitlements, credits, invoices and
affiliates, and broadcasts domain events back out through the transactional outbox.
...
| `outreach/` | AI retention engine: trigger scheduler → LLM generation (template fallback) → notify → conversion attribution. See [ai-features.md](/docs/ai-features). |
...
`applyStateTransition(paymentIntentId, status)` is the heart of the service. It runs inside a
single `$transaction`, validates the edge (invalid transitions throw), and re-applying the
current state is a no-op. The **PaymentIntent** is the source of truth; the **Order** status is
a projection of it.
...
`refunded→refunded`, `failed→failed`, `expired→expired`) — so its reachable states are:
...
Checkout is deliberately two-phase so the order exists (and can be risk-scored, funnel-tracked
and abandoned-recovered) before any payment intent is created. **Promo application and intent
creation happen only in phase 2**, inside one transaction that atomically increments promo usage
under a `WHERE currentUsageCount < max` guard.
...
**Inbound — provider webhooks.** Every provider posts to a per-rail endpoint that verifies the
...
transition, so an underpayment or currency mismatch downgrades the intent to `failed` rather
than fulfilling.
...
**Outbound — transactional outbox.** Domain code writes an `OutboxEvent` in the *same*
transaction as the state change (so an event is never emitted for a change that rolled back).
The processor drains pending events and delivers each to every active service webhook, guarded
by an SSRF check that rejects private/link-local hosts.
...
New subscriptions start `trialing` when the price has trial days, else `active`. Renewals arrive
as provider webhooks (`subscription.renewed` synthesises a paid order and reuses the fulfilment
path). What providers don't tell us, the **hourly** `SubscriptionExpirerScheduler` reconciles:
it pushes overdue provider-backed subs to `past_due`, opens the grace window (`Price.graceDays`),
and ends them if grace is exhausted.
...
`Timestamptz(6)`. Money is `Decimal(19,4)` — never floats; credits are integers. The commerce
spine below is fully relational; note that **`userId` is an external string** (no local User
table — identity lives in `auth.inite.ai`), and **`Entitlement` links to its source order or
subscription softly, through its `value` JSON** rather than a foreign key.
...
- Credits: `CreditBalance`, `CreditUsage` (+ `featureCode`/`units`/`modelTier`), `MeteredFeature`,
  `FeatureQuota`.
- AI: `Conversation`, `ChatMessage` (feedback columns), `AssistantToolCall`, `AssistantAction`,
  `ProductEmbedding` (vector 1536), `AiInsight`.
