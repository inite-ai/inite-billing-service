...
| BNB Chain (BEP-20) | USDT, USDC | 15 confirmations |
| Polygon PoS | USDT, USDC | 64 confirmations |
| Arbitrum One | USDT, USDC | 20 confirmations |
| Optimism | USDT, USDC | 20 confirmations |
| Base | USDC, USDT | 20 confirmations |
| Avalanche C-Chain | USDT, USDC | 12 confirmations |

All EVM networks share **one 0x wallet** — the same address works on every one of them — and are
watched straight from the chain (`Transfer` logs over public RPC, no API key). How far each has been
read is kept in the database, so a restart resumes instead of missing blocks. A network's RPC can be
overridden per network in the settings.
...
2. **Chain access** — No keys are needed; a TronGrid or Toncenter key raises their rate limits. Keys are stored
   write-only.
...
public API for recent incoming transfers — TronGrid, Toncenter, Solana RPC and each EVM network's RPC. An idle
