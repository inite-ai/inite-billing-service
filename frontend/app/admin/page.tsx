...
export default function AdminDashboardPage() {
  ...
  if (loading) return <div className="flex items-center gap-2 text-slate-500"><Loader2 className="w-5 h-5 animate-spin" /> {tc('loadingDashboard')}</div>
  if (!stats) return <div className="text-slate-500">{t('dashboard.failedToLoadStats')}</div>
        ...
        <div className="lg:col-span-2">
          <Card>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-slate-900 dark:text-white">{t('dashboard.recentOrders')}</h3>
              <Link href="/admin/orders">
                <Button size="sm" variant="ghost" icon={<ArrowRight className="w-4 h-4" />}>{tc('viewAll')}</Button>
              </Link>
            </div>
            {recentOrders.length === 0 ? (
              <p className="text-slate-500 text-sm py-4">{t('dashboard.noOrdersYet')}</p>
            ) : (
              <Table>
                <Thead>
            ...
            {quickActions.map((action) => {
              ...
              return (
                <Link key={action.href} href={action.href}>
                  <div className="flex items-center gap-3 p-3 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors cursor-pointer">
