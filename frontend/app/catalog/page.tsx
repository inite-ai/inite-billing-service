...
function CatalogContent() {
  ...
  const currency = useMemo(() => {
    ...
    return resolveCurrency(currencies, chosenCurrency, readStoredCurrency(), guessCurrency(locale, browser))
  }, [currencies, chosenCurrency, locale])

  const plans = visibleProducts.filter((p) => p.type === 'subscription')
  const hasYearly = plans.some((p) => priceFor(p, currency, 'year') && priceFor(p, currency, 'month'))
    ...
    <LedgerShell wide nav={nav}>
      <div className="wrap">
        <header className="rise d1 pb-8 pt-14 sm:pt-20">
          ...
          <p className="mt-5 max-w-[46ch] text-[17px] leading-relaxed text-[color:var(--dim)]">{t('lead')}</p>
        </header>
        ...
        {services && services.length > 1 && (
          <div className="svc-tabs mb-4" role="group" aria-label={t('allProducts')}>
            ...
            {services.map((s) => (
              <button key={s.code} type="button" aria-pressed={serviceCode === s.code} onClick={() => setService(s.code)}>
                {s.name}
          ...
          <div className="mb-10 flex flex-wrap items-center gap-3 border-b border-[color:var(--line)] pb-6">
            ...
            {currencies.length > 1 && (
              <div className="seg sm:ml-auto" role="group" aria-label={t('currency')}>
                {currencies.map((c) => (
                  <button key={c} type="button" aria-pressed={currency === c} onClick={() => pickCurrency(c)}>
                    {c}
                  </button>
                ))}
              </div>
        ...
        {failed ? (
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3" aria-busy="true" aria-label={tc('loading')}>
            {[0, 1, 2].map((i) => (
              <div key={i} className="skel h-[420px]" />
            ))}
          </div>
        ) : visible.length === 0 ? (
          <p className="panel mx-auto max-w-md p-9 text-center text-[color:var(--dim)]">{t('noProducts')}</p>
        ) : (
...
function ServiceSection({
  ...
  const plans = service.products.filter((p) => p.type === 'subscription' && !isAddon(p)).sort(byPrice)
  const addons = service.products.filter((p) => p.type === 'subscription' && isAddon(p)).sort(byPrice)
    ...
    <section aria-labelledby={`svc-${service.code}`}>
      {showHeading && (
        <div className="sechead !mb-8">
          ...
          <span className="hint mono !text-[12px]">{t('productsCount', { count: service.products.length })}</span>
        </div>
      )}
      ...
      <div className="space-y-12">
        {plans.length > 0 && (
          <div>
            ...
            <div className={`grid gap-4 pt-3 sm:grid-cols-2 ${PLAN_COLUMNS[Math.min(plans.length, 4)]}`}>
              {plans.map((p) => (
                <PlanCard
                  key={p.id}
                  product={p}
        ...
        {oneOff.length > 0 && (
          <div>
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {oneOff.map((p) => (
                <OneOffCard key={p.id} product={p} currency={currency} busy={busyIn(p)} onBuy={onBuy} />
              ))}
            </div>
          </div>
        )}
