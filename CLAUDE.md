...
- `adapters/crypto/` + `workers/crypto-watcher.scheduler.ts` + `crypto/` (admin) — stablecoins (USDT/USDC on TRON, TON, Solana and seven EVM networks sharing one 0x wallet, watched via eth_getLogs with a DB cursor) straight to own wallets. No memo on ERC-20/TRC-20, so each `CryptoInvoice` reserves a unique amount (partial unique index); the watcher polls chain APIs every 30s and turns matched transfers into ordinary webhook events; unmatched ones become `CryptoTransfer` rows for admin assignment. Non-USD prices are converted via `FxRates` (open.er-api.com → CBR fallback, admin-pinned rates win, markup %) and the rate is fixed on the invoice.
